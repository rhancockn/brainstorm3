...
% Modified to support NIRx LSL/XDF by Roeland Hancock, 2024
...
for iStream = 1:length(streams)
    if strcmp(streams{iStream}.info.name, 'NSP3')
        streams(iStream) = [];
    end
end
...
for iStream = 1:length(streams)
    ...
    nChannels(iStream) = 0;
    for iChan = 1:size(stream.time_series,1)
        if strcmp(stream.info.desc.channels.channel{iChan}.type, 'nirs_hbr') || strcmp(stream.info.desc.channels.channel{iChan}.type, 'nirs_hbo') || strcmp(stream.info.desc.channels.channel{iChan}.type, 'nirs_frame')
            continue
        end
        if isfield(stream.info.desc, 'channels') && isfield(stream.info.desc.channels.channel{iChan}, 'label') && ~isempty(stream.info.desc.channels.channel{iChan}.label)
            chNames{end+1} = [replace(stream.info.desc.channels.channel{iChan}.custom_name, '-', '') 'WL' stream.info.desc.channels.channel{iChan}.wavelength(1:3)];
        else
            continue
        end
        if strfind(stream.info.desc.channels.channel{iChan}.type, 'nirs_raw')
            chTypes{end+1} = 'NIRS';
        elseif isfield(stream.info.desc, 'channels') && isfield(stream.info.desc.channels.channel{iChan}, 'type') && ~isempty(stream.info.desc.channels.channel{iChan}.type)
            chTypes{end+1} = stream.info.desc.channels.channel{iChan}.type;
        else
            chTypes{end+1} = 'Unknown';
        end
        nChannels(iStream) = nChannels(iStream) + 1;
        chGroups{end+1} = ['WL' stream.info.desc.channels.channel{iChan}.wavelength(1:3)];
        chWavelengths(end+1) = str2double(stream.info.desc.channels.channel{iChan}.wavelength);

        sourceDector = regexp(stream.info.desc.channels.channel{iChan}.custom_name, 'S(?<source>\d+)-D(?<detector>\d+)', 'names');
        source = int32(str2num(sourceDector.source));
        detector = int32(str2num(sourceDector.detector));

        chLocs{end+1} = [str2double(stream.info.desc.montage.optodes.sources.source{source}.location.x) ...
                        str2double(stream.info.desc.montage.optodes.sources.source{source}.location.y) ... 
                        str2double(stream.info.desc.montage.optodes.sources.source{source}.location.z);
                        str2double(stream.info.desc.montage.optodes.detectors.detector{detector}.location.x) ...
                        str2double(stream.info.desc.montage.optodes.detectors.detector{detector}.location.y) ...
                        str2double(stream.info.desc.montage.optodes.detectors.detector{detector}.location.z)]'/1e3;
    end
end
...
for i = 1:nChannels
    ...
    ChannelMat.Channel(i).Loc     = chLocs{i};
    ChannelMat.Channel(i).Group   = chGroups{i};
    ChannelMat.Channel(i).Orient  = [];
    ChannelMat.Channel(i).Weight  = 1;
    ChannelMat.Channel(i).Comment = [];
end
ChannelMat.Nirs.Wavelengths = chWavelengths';

%% fudicials
ChannelMat.SCS.NAS = [0.400 85.900 -47.600]/1e3;
ChannelMat.SCS.LPA = [-83.800 -18.600 -57.200 ]/1e3;
ChannelMat.SCS.RPA = [83.900 -16.600 -56.700 ]/1e3;
ChannelMat.HeadPoints.Loc = [ChannelMat.SCS.NAS; ChannelMat.SCS.LPA; ChannelMat.SCS.RPA; [0.200 -120.500 -25.800]/1e3]';
ChannelMat.HeadPoints.Label = {'NAS', 'LPA', 'RPA', 'Iz'};
ChannelMat.HeadPoints.Type = {'CARDINAL', 'CARDINAL', 'CARDINAL', 'EXTRA'};

% trigger coordinate transform calculation
...
for iStream = find(srates == 0)
    s = streams{iStream};
    % Convert numeric markers to strings
    if strcmpi(s.info.type, 'Markers') && ~iscell(s.time_series)
        s.time_series = arrayfun(@num2str, s.time_series, 'UniformOutput', false);
    end
